...
public class CameraStreamServiceImpl implements CameraStreamService {
    ...
    private void saveLastFrame(Camera camera, String cameraId) {
        try {
            String outputPath = Paths.get("output", cameraId, "snapshot.jpg").toString();
            ...
            camera.setSnapshotUrl(urlUtil.getBaseUrl(request) + "/output/" + camera.getId() + "/snapshot.jpg");
