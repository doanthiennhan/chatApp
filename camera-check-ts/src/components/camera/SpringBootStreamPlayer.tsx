...
import { Card, Button, message, Badge, Descriptions, Spin, Tooltip, Space, Tag } from 'antd';
import { 
  PlayCircleOutlined, 
  StopOutlined, 
  EyeOutlined, 
...
interface SpringBootStreamPlayerProps {
  camera: Camera;
...
const SpringBootStreamPlayer: React.FC<SpringBootStreamPlayerProps> = ({ camera, selectedCamera, isInModal = false, visible = true }) => {
  ...
  const [videoQuality, setVideoQuality] = useState<string>('Unknown');
  const [cameraLocation, setCameraLocation] = useState<string>('');
  const [localRealTimeStatus, setLocalRealTimeStatus] = useState<LocalRealTimeStatus>({
    isOnline: false,
    lastSeen: null,
    uptime: 0,
    packetLoss: 0,
  ...
  const playerRef = useRef<typeof JSMpegPlayer | null>(null);
    ...
    metadataWebSocketRef.current = createWebSocketConnection(
      ...
      (event: MessageEvent) => {
        ...
        try {
          ...
          console.error("Error parsing metadata websocket message:", e);
          return;
        }
        if (parsed) {
          ...
          setLocalRealTimeStatus((prev: LocalRealTimeStatus) => ({
            ...prev,
      ...
      (err: Event) => console.error(' Metadata error:', err)
    );
  };
  ...
  const handleStartStream = async () => {
    ...
    try {
      ...
      let player;
      try {
        player = new JSMpegPlayer(wsUrl, playerOptions);
      } catch (err) {
        console.error('🔧 Failed to create JSMpeg player with WebGL disabled, trying with basic options:', err);
        ...
        player = new JSMpegPlayer(wsUrl, basicOptions);
      }
      if (player) {
        ...
        player.onError = (playerInstance: typeof JSMpegPlayer, error: any) => {
      ...
      setConnectionStatus('error');
      setLoading(false);
      message.error('Lỗi khi bắt đầu stream: ' + err.message);
    }
  };

  const getStatusColor = (): 'success' | 'default' | 'error' | 'processing' => {
  ...
  const getQualityColor = (quality: string): 'green' | 'blue' | 'orange' | 'red' | 'gray' => {
    switch (quality) {
